...
function Test(){
    ...
    let wrong = 0;
    let testdata;
    if(window.localStorage.getItem("testdata")){
        testdata = JSON.parse(window.localStorage.getItem("testdata"))
    } else {
        testdata = {
            letters:[" ","f","d","j","k"],
            length:"30"
    ...
    const boxs = array.map(e=>{
        ...
        <input className={classbox}   onChange={(ele)=>{
            ...
            if(`${nextbox}` !== `box${array.length + 1}`){
                ...
                h1.appendChild(document.createTextNode("Test Done"));
    ...
    return(
        <div className="lesson">
            ...
            <h1 className='lessontitle'>Start Test</h1>
            <h2 className='info'>
